...
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
'
' GenerateDataFolderInformation
'
' Purpose: 
'
' prints the contents and state of the data folder
' 
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''

Sub GenerateDataFolderInformation(Folder)
	...
	Set Files = Folder.Files
	If 1 = Files.Count Then
    Print TabStop & Folder.Path & " (1 file)"
	Else
    Print TabStop & Folder.Path & " (" & Files.Count & " files)"
	End If
	...
	If 1 = SubFolders.Count Then
    Print NewLine & "There is 1 sub folder" & NewLine
	Else
    Print NewLine & "There are " & SubFolders.Count & " sub folders" & NewLine
	End If

	If SubFolders.Count <> 0 Then

		For Each SubFolder In SubFolders
      Print  GenerateFolderInformation(SubFolder)
		Next

    Print NewLine

		For Each SubFolder In SubFolders
      GenerateDataFolderInformation(SubFolder)
		Next
...
Sub Main
  ...
  Print "Commandline Arguments:" & NewLine
  For i = 0 to objArgs.Count - 1
    Print objArgs(i) & NewLine
  Next

  Print "Scriptname: " & WScript.ScriptName & NewLine
  Print "ScriptFullname: " & WScript.ScriptFullname & NewLine
  
  TestFilePath = Left(WScript.ScriptFullname,Len(WScript.ScriptFullname)-Len(WScript.ScriptName))
  TestDrive = Left(TestFilePath,1)
  
  Print "Drive: " & TestDrive & NewLine
  Print "Path: " & TestFilePath & NewLine

  Set FSO = CreateObject("Scripting.FileSystemObject")
  If Not FSO.FolderExists(TestFilePath) Then Exit Sub
  GenerateDataFolderInformation FSO.GetFolder(TestFilePath)
